...
async def clear_table(ctx):
  ...
  if connection:
    row = await Module.Kernel.Authentication.IsCaptain(ctx ,'/captain clear_table', connection, ctx.guild.id, ctx.author.id)
    if row:
      group_serial = row[0]
      if await Module.Kernel.Authentication.IsSignChannel(ctx,connection,group_serial):
        ...
        # 重設boss次序
        cursor = connection.cursor(prepared=True)
        sql = "UPDATE princess_connect.group SET now_week='1', now_week_1='1', now_week_2='1', now_week_3='1', now_week_4='1', now_week_5='1' WHERE server_id = ? and group_serial=?"
        data = (ctx.guild.id, group_serial)
        cursor.execute(sql, data)
        cursor.close
        connection.commit()
