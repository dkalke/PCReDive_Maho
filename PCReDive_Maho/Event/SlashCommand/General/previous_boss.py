  ...
  if connection:
    ...
    if row:
      ...
      if boss_change[boss-1] <= ctx.created_at + datetime.timedelta(hours = 8):
        if ctx.created_at + datetime.timedelta(hours = 8) <= boss_change[boss-1] + datetime.timedelta(seconds = Module.Kernel.define_value.NCD_TIME): 
          # 更新該王週目與主週目
          ...
          data = (now_week[boss-1], new_main_week, (boss_change[boss-1] + datetime.timedelta(seconds = Module.Kernel.define_value.NCD_TIME)).strftime("%Y-%m-%d %H:%M:%S"), ctx.guild.id, ctx.channel.id)
          cursor.execute(sql, data)
          cursor.close
          connection.commit()

          await ctx.send(str(boss) + '王已退回' + str(now_week[boss-1]) + '週目，請務必提醒受影響成員，以免誤入錯誤週目!')
          Module.Kernel.Offset_manager.AutoOffset(connection, ctx.guild.id, group_serial) # 自動周目控制
