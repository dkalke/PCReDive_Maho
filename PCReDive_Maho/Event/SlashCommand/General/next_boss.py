  ...
  if connection:
    ...
    if row:
      ...
      if ( not now_week[boss-1] >= main_week + 2 ) and (Module.Kernel.week_stage.week_stage(now_week[boss-1]) == Module.Kernel.week_stage.week_stage(main_week)):
        if ctx.created_at + datetime.timedelta(hours = 8) >= boss_change[boss-1] + datetime.timedelta(seconds = Module.Kernel.define_value.CD_TIME): 
          # 更新該王週目
          ...
          if new_main_week > main_week:
            knifes = await Module.Kernel.show_knifes.show_knifes(connection, ctx, group_serial, now_week[boss-1] ,boss)
            ...
            for index in boss_index:
              if new_main_week == Module.Kernel.define_value.Stage.one.value or \
                ...
                knifes = await Module.Kernel.show_knifes.show_knifes(connection, ctx, group_serial, now_week[index-1] ,index)
